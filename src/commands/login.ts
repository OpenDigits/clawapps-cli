...
export async function loginCommand(options: { human?: boolean }): Promise<void> {
  ...
  if (isHuman) {
    ...
    // JSON mode: QR text first, then JSON metadata
  ...
  const spinner = isHuman ? ora('Waiting for authentication...').start() : null;
